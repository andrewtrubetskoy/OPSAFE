...
function renderRiskCardThreatsTable(m) {
    ...
    const getProbText = c => ({ F:'Дуже часто', L:'Часто', O:'Можливо', U:'Рідко', ND:'Не визначено' }[c] || 'Не визначено');
    let html = '', counter = 1;
    m.data.database.forEach(item => {
        ...
        if (item.type === 'primary') {
            ...
            if (item.secondaries) item.secondaries.forEach((sec,si) => { 
                ...
                html+=`<tr class="bg-gray-50"><td class="border border-gray-400 p-1 text-center text-xs text-gray-500 align-top">${sstHtml}</td><td class="border border-gray-400 p-1 pl-3 text-xs italic text-gray-700">${sec.name}</td><td class="border border-gray-400 p-1"><div class="flex items-center justify-start gap-1.5"><span class="font-bold px-1.5 py-0.5 rounded text-white text-xs whitespace-nowrap" style="background:${getProbBg(sp)}">${sp}</span><span class="text-[10px] font-bold leading-tight text-left" style="color:${getProbBg(sp)}">${getProbText(sp)}</span></div></td><td class="border border-gray-400 p-1 text-xs">${sc}</td><td class="border border-gray-400 p-1 text-sm"><div class="text-[11px] text-gray-500">Хто:</div><input type="text" class="w-full border-b border-dashed border-gray-400 bg-transparent outline-none text-sm"></td><td class="border border-gray-400 p-1 text-center"><span class="font-bold px-1 py-0.5 rounded text-white text-xs" style="background:${getProbBg(srp)}">${srp}</span></td><td class="border border-gray-400 p-1 text-center"><input type="checkbox" class="w-4 h-4"></td></tr>`; 
            });
            counter++;
        } else if (item.type === 'secondary_indep') {
            ...
            html+=`<tr><td class="border border-gray-400 p-1 text-center font-bold text-xs text-orange-700 align-top w-24">${istHtml}</td><td class="border border-gray-400 p-1 text-sm text-orange-800">${item.name}</td><td class="border border-gray-400 p-1"><div class="flex items-center justify-start gap-1.5"><span class="font-bold px-1.5 py-0.5 rounded text-white text-xs whitespace-nowrap" style="background:${getProbBg(p)}">${p}</span><span class="text-[10px] font-bold leading-tight text-left" style="color:${getProbBg(p)}">${getProbText(p)}</span></div></td><td class="border border-gray-400 p-1 text-xs">${c}</td><td class="border border-gray-400 p-1 text-sm"><div class="text-[11px] text-gray-500">Хто:</div><input type="text" class="w-full border-b border-dashed border-gray-400 bg-transparent outline-none text-sm mb-1"><input type="text" class="w-full border-b border-dashed border-gray-400 bg-transparent outline-none text-sm" placeholder="уточнення..."></td><td class="border border-gray-400 p-1 text-center"><span class="font-bold px-1 py-0.5 rounded text-white text-sm" style="background:${getProbBg(rp)}">${rp}</span></td><td class="border border-gray-400 p-1 text-center"><input type="checkbox" class="w-4 h-4"></td></tr>`;
            counter++;
        }
    });
